...
echo '===== launch forced-userspace AmneziaWG 3.1 ====='
docker run -d \
...
docker exec "$NAME" sh -lc '
  ip -d link show awg0 | head -8
  awg show awg0
' | sed -E 's/(private key: ).*/\1(hidden)/'
...
echo '===== authenticated AWG3 end-to-end test ====='
set +e
TEST_OUT="$(docker run --rm \
  ...
  -v "$STATE:/config:ro" --entrypoint /bin/sh "$IMAGE" -lc '
    ...
    echo HTTP
    wget -qO- -T 15 http://1.1.1.1/cdn-cgi/trace | grep -E "^(ip|warp)="
    echo TRANSFER_AFTER
    awg show awg0 transfer
  ' 2>&1)"
STATUS=$?
...
# Remove only the temporary VLESS service created during the earlier wrong turn.
