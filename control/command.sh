...
# Remove only our failed/previous mobile AWG3 container before rebuilding it.
...
import secrets
v=sorted(secrets.SystemRandom().sample(range(100000, 2_000_000_000), 8))
print(*(f'{v[i]}-{v[i+1]}' for i in range(0,8,2)))
PY
)
S1=83; S2=117; S3=41; S4=17

cat > "$STATE/awg0.conf" <<EOF
[Interface]
...
rm -f "$STATE/server.conf"

cat > "$STATE/start-server.sh" <<'EOF'
#!/bin/sh
set -eu
awg-quick down /config/awg0.conf >/dev/null 2>&1 || true
...
trap 'awg-quick down /config/awg0.conf >/dev/null 2>&1 || true; exit 0' TERM INT
...
docker run --rm --cap-add NET_ADMIN --device /dev/net/tun \
  ...
  'awg-quick strip /config/awg0.conf >/tmp/stripped && grep -q HeaderProtectionKey /tmp/stripped && echo server_config=ok'
...
docker run -d \
  ...
  -p "$PORT:$PORT/udp" \
  -v "$STATE:/config:ro" \
  --entrypoint /bin/sh "$IMAGE" /config/start-server.sh >/dev/null
sleep 3
...
TEST_OUT="$(docker run --rm \
  ...
  -v "$STATE:/config:ro" --entrypoint /bin/sh "$IMAGE" -lc '
    set -e
    cp /config/test-client.conf /tmp/awg0.conf
    chmod 600 /tmp/awg0.conf
    awg-quick up /tmp/awg0.conf >/dev/null
    sleep 2
    echo HANDSHAKE
    ...
    awg-quick down /tmp/awg0.conf >/dev/null
  ' 2>&1)"
TEST_STATUS=$?
set -e
printf '%s\n' "$TEST_OUT"
...
# Clean up only the temporary VLESS container from the previous wrong approach.
